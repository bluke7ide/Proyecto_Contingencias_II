...
```{r}
t <- proc.time()
proyecciones_totales_parallel <- ejecutar_proyecciones_par(100, 0.07, 0.03)
proc.time()-t
```
...
proyecciones <- list()
for( i in 1:100){
  proyecciones[[i]] <- proy_prima(0.07,0.03)
}
proc.time() - t
```
...
WARNING: En progreso. 
```{r}
proy_vp <- function(interes, inflacion) {
  # Inicialización de los valores
  estados <- rep(1, 5000)
  ...
  for (i in 1:rango) {
    # Accede directamente a las probabilidades acumuladas en `lista`
    locales <- sapply(1:5000, function(x) {
      if (i <= max_iters[x] && estados[x] < 6) {
        lista[[portfolio$id[x]]][[estados[x]]][i, ]
      } else {
        rep(NA, 6)
      }
    })
    ...
    # Calcula los nuevos estados
    nuevos_estados <- sapply(1:5000, function(j) {
      estado <- estados[j]
      if (estado == 6) {
        return(estado)
      }
    ...
    # Actualización de estados y valores presentes de estados muertos
    estados <- ifelse(estados == 6 & vp[cbind(1:5000, 6)] == 0, estados, nuevos_estados)
    vp[estados == 6 & vp[cbind(1:5000, 6)] == 0, 6] <- v_power[i + 1]
  }
  vector <- lista[[1]]$Able[100,]
  vector[1:6] <- sapply(as.data.frame(vp), sum)
  return(vp)
}
```
...
```{r}
t <- proc.time()
proy_vp_data <- proy_vp(0.07, 0.03)
proc.time()-t
```
...
proy_vivos <- function(){
  ...
  rango <- 120 - min(edades)
  unifs <- matrix(runif(rango * 5000), nrow = rango, ncol = 5000)
  vivos <- matrix(0, nrow = 101, ncol = 6)
  vivos[1, 1] <- 5000
  max_iters <- 120 - edades

  for (i in 1:rango) {
    # Filtra las probabilidades para los estados activos
    locales <- sapply(1:5000, function(x) {
      if (i <= max_iters[x] && estados[x] < 6) {
        lista[[portfolio$id[x]]][[estados[x]]][i, ]
      } else {
    ...
    estados <- ifelse(estados == 6, estados, nuevos_estados)
    vivos[i+1, ] <- tabulate(estados, nbins = 6)
  }

  # Convertimos vivos a data frame
  vivos_df <- as.data.frame(cbind(0:100, vivos))
  colnames(vivos_df) <- c("x", "Able", "Mild", "Moderate", "Severe", "Profound", "Dead")
  return(vivos_df)
}
