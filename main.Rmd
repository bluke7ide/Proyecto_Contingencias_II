...
proyeccion <- function(interes, inflacion){
  ...
  for (i in 1:rango) {
    ...
    locales <- sapply(1:102, function(x) {
      edad_actual <- edades[x]
      max_iter <- 120 - edad_actual
      if (i <= max_iter && estados[x] < 6) {
        lista[[x]][[estados[x]]][i, 2:7]
      } else {
        rep(NA, 6)
      }
    })
    # Pone los valores presentes para los estados vivos
    valid_states <- estados < 6
    vp[cbind(1:102, estados)] <- vp[cbind(1:102, estados)] + ifelse(valid_states, v^(i - 1), 0)
    ...
    # Pone los valores presentes para los estados muertos
    estados <- ifelse(estados == 6 & vp[cbind(1:102, 6)] == 0, estados, nuevos_estados)
    vp[estados == 6, 6] <- ifelse(vp[estados == 6, 6] == 0, v^i, vp[estados == 6, 6])
  }
  return(vp)
}
```
